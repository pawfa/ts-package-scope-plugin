...
export function isPublicScoped(info: PluginCreateInfo, fileName: string, importPath: string, ts: TypeScript) {
  ...
  const importedSourceFile = getSourceFile(
    info.languageService,
    importedFileName + (importedFileName.endsWith(".ts") ? "" : ".ts")
  );

  const importFilePackageJsDocTags: JSDocTag[] = [];

  importedSourceFile?.forEachChild((node) => {
    const packageJsDocTagsForNode = ts
      .getJSDocTags(node)
      .filter((tag) => tag.tagName.escapedText.toString().includes("package-"));
